...
# JUnit4: Experiments 3 + 5 with 7B model (rerun)
...
#
# Usage:  nohup ./run_junit4_exp3_exp5_7b.sh > /dev/null 2>&1 &
# Monitor:
#   tail -f ~/repos/gin-rl/experiment_run_bg.log | grep --line-buffered -E "(junit4|Exp [35]|done|WARNING|complete|7b)"
...
echo "" | tee -a "$LOG"
echo "=== Starting JUnit4 Exp3+Exp5 (7b model, sequential) at $(date) | Timestamp: $TIMESTAMP ===" | tee -a "$LOG"
...
run_exp3_rep() {
    ...
    local output_file="$RESULTS_DIR/exp3_ucb_all_7b_rep${rep}_${TIMESTAMP}.csv"
    local rl_log_file="$RESULTS_DIR/exp3_rl_log_7b_rep${rep}_${TIMESTAMP}.csv"

    echo "--- JUnit4 Exp3-7b Rep $rep started (seed=$seed) ---" | tee -a "$LOG"
    ...
    if [ $exit_code -eq 124 ] || [ $exit_code -eq 143 ]; then
        echo "WARNING: JUnit4 Exp3-7b Rep $rep timed out" | tee -a "$LOG"
    else
        echo "JUnit4 Exp3-7b Rep $rep done: $output_file" | tee -a "$LOG"
    fi
}
...
run_exp5_rep() {
    local rep=$1
    local seed=$((123 + rep * 1000))
    local output_file="$RESULTS_DIR/exp5_ls_all_7b_rep${rep}_${TIMESTAMP}.csv"

    echo "--- JUnit4 Exp5-7b Rep $rep started (seed=$seed) ---" | tee -a "$LOG"
    ...
    local exit_code=$?
    if [ $exit_code -eq 124 ] || [ $exit_code -eq 143 ]; then
        echo "WARNING: JUnit4 Exp5-7b Rep $rep timed out" | tee -a "$LOG"
    else
        echo "JUnit4 Exp5-7b Rep $rep done: $output_file" | tee -a "$LOG"
    fi
}
...
echo "=== JUnit4 Exp3+Exp5 (7b) complete at $(date) ===" | tee -a "$LOG"
