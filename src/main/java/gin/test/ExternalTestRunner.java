...
public class ExternalTestRunner extends TestRunner {
    ...
    private List<UnitTestResult> runTests(int reps) throws IOException, InterruptedException {
        ...
        while (index < maxIndex) {
            ...
            // Register a shutdown hook so the subprocess is killed if our JVM
            // exits unexpectedly. We track the Thread so we can deregister it
            // after normal cleanup — without this, hooks accumulate (one per step)
            // and exhaust the OS thread limit over a long run.
            ...
            try {
                ...
                // to prevent file descriptor leaks over many steps.
                try { client.stopConnection(); } catch (Exception ignored) {}

                try { Thread.sleep(500); } catch (InterruptedException ignored) {} // cleanup time

                // Kill descendant processes first (grandchildren of this JVM).
                // destroyForcibly() only kills the direct subprocess; any processes
                // it spawned become orphans and accumulate toward the OS process limit
                // over long runs with LLM operators.
                ...
                // Deregister the shutdown hook now that we've cleaned up the process.
                // This prevents unbounded accumulation of hook threads over long runs.
