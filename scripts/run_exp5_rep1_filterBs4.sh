...
METHOD_FILE="$PROJECT_DIR/methods/jcodec_wang_filterBs4.csv"
...
TEMP_FILE="$RESULTS_DIR/exp5_rep1_filterBs4_temp.csv"

echo "" | tee -a "$LOG"
echo "Starting Exp 5 Rep 1 filterBs4 redo at $(date)" | tee -a "$LOG"
...
if [ $exit_code -eq 124 ] || [ $exit_code -eq 143 ]; then
    ...
    echo "Exp 5 Rep 1 filterBs4 done." | tee -a "$LOG"
fi

if [ -f "$TEMP_FILE" ]; then
    tail -n +2 "$TEMP_FILE" >> "$REP1_FILE"
    rm "$TEMP_FILE"
    echo "Appended filterBs4 to: $REP1_FILE" | tee -a "$LOG"
fi
