...
# JUnit4 Experiment 1 RERUN 2: Random Sampling pn=1000, 5 reps
#
# Usage:  nohup ./run_junit4_exp1_rerun2.sh > /dev/null 2>&1 &
...
echo "" | tee -a "$LOG"
echo "=== Starting JUnit4 Exp1 RERUN2 (pn=1000) at $(date) | Timestamp: $TIMESTAMP ===" | tee -a "$LOG"
...
run_exp1_rep() {
    ...
    echo "--- JUnit4 Exp1-R2 Rep $rep started (seed=$seed) ---" | tee -a "$LOG"
    ...
    if [ $exit_code -eq 124 ] || [ $exit_code -eq 143 ]; then
        echo "WARNING: JUnit4 Exp1-R2 Rep $rep timed out" | tee -a "$LOG"
    else
        echo "JUnit4 Exp1-R2 Rep $rep done: $output_file" | tee -a "$LOG"
    fi
}
...
echo "=== JUnit4 Exp1 RERUN2 complete at $(date) ===" | tee -a "$LOG"
